...
func (s *Chronos) Less(podInfo1, podInfo2 *framework.QueuedPodInfo) bool {
	...
	if aged1 != aged2 {
		klog.V(4).Infof("QueueSort: Max queue age decision - %s(aged=%t) vs %s(aged=%t) = %t",
			podInfo1.Pod.Name, aged1, podInfo2.Pod.Name, aged2, aged1)
		return aged1
	}
	if aged1 {
		queuedAt1 := *podInfo1.InitialAttemptTimestamp
		queuedAt2 := *podInfo2.InitialAttemptTimestamp
...
func (s *Chronos) exceededMaxQueueAge(podInfo *framework.QueuedPodInfo, now time.Time) bool {
	queuedAt := *podInfo.InitialAttemptTimestamp
